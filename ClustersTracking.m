...
function ClustersTracking(videoname)
...
% Save the results. (tracker ID, frame#, [1,2: x,y position. 3: cluster area. 4,5: W,H box size])
Results = zeros(1,1,5);
...
% foreground objects, and displaying results.
    ...
    function obj = setupSystemObjects(fname)
        ...
        obj.reader = vision.VideoFileReader(fname,'ImageColorSpace','RGB','VideoOutputDataType','uint8');
        ...
        res_out=strcat(char(fn(1)),'_Result.',char(fn(2)));
        msk_out=strcat(char(fn(1)),'_Mask.',char(fn(2)));
        obj.Result_out=VideoWriter(res_out,'Motion JPEG AVI');
        obj.Mask_out= VideoWriter(msk_out,'Motion JPEG AVI');
        obj.Result_out.FrameRate=20;
        obj.Mask_out.FrameRate=20;
        open(obj.Result_out);
        open(obj.Mask_out);
        
        obj.blobAnalyser = vision.BlobAnalysis('BoundingBoxOutputPort', true, ...
            'AreaOutputPort', true, 'CentroidOutputPort', true, ...
            'MinimumBlobArea', 40, 'MaximumCount', 150);
    end
...
%                 Results(max(ids),:,1:5)=zeros;
                Results(ids,CurrentFrame,1)=bboxes(:,1)+bboxes(:,3)/2;
                Results(ids,CurrentFrame,2)=bboxes(:,2)+bboxes(:,4)/2;
%                 Results(ids,CurrentFrame,3)=temp(ids);
                Results(ids,CurrentFrame,4)=bboxes(:,3);
                Results(ids,CurrentFrame,5)=bboxes(:,4);
        ...
        assignin('base','Result',Results);
        % Display and save the mask and the frame.
        obj.maskPlayer.step(mask); 
        writeVideo(obj.Mask_out,mask);
        obj.videoPlayer.step(frame);
        writeVideo(obj.Result_out,frame);
    end
end
